...
import { BookOpen, CalendarDays, ChevronLeft, ChevronRight, CircleHelp, FileText, GraduationCap, LogOut, Moon, Plus, Search, Settings, Sparkles, Sun, Trash2, X } from "lucide-react";
...
export default function Page() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      <header className="topbar"><div className="breadcrumb">{activeEnrollment ? <><button onClick={() => setActiveEnrollment(null)}>Courses</button><ChevronRight size={15}/><span>{currentCourse?.name_mk}</span></> : <span>{nav.find(item => item.id === view)?.label}</span>}</div><span className="topbar-meta">VIRGIL</span></header>
      <div className="page-content">
        ...
        {view === "courses" && activeEnrollment && currentCourse && <>
          <div className="course-hero"><div><div className="eyebrow">{selectedTerm?.label || "COURSE"}</div><h1>{currentCourse.name_mk}</h1><p>Your books, materials, lectures, and practice in one place.</p></div><div className="hero-symbol"><BookOpen size={32}/></div></div>
...
function BrandLogo() { return <Image className="brand-logo" src="/assets/virgil-logo.png" alt="Virgil logo" width={40} height={40} priority/>; }
...
function SettingsView({ session, client, theme, setTheme }: { session: Session; client: SupabaseClient; theme: "light" | "dark"; setTheme: (theme: "light" | "dark") => void }) {
  return <>
    <div className="page-heading"><div><div className="eyebrow">PREFERENCES</div><h1>Settings</h1><p>Adjust how your workspace looks and manage your account.</p></div></div>
    ...
    <div className="panel settings-panel"><h2>About Virgil</h2><p>Virgil is an independent student project and is not affiliated with FINKI. The SEIS23 course catalog is available.</p></div>
  </>;
}
